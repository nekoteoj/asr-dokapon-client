...
if [ $stage -le 1 ]; then
  ...
  utils/subset_data_dir.sh data/train_hires 200 data/train_hires_30k
  # want the 100k subset to exactly match train_100k, since we'll use its alignments.
  awk '{print $1}' data/train.1k/utt2spk > uttlist
...
if [ $stage -le 3 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 --num-frames 400000 \
    data/train_hires_30k 10 exp/nnet2_online/tri5a exp/nnet2_online/diag_ubm
fi
