...
utils/prepare_lang.sh data/local/dict_test "<UNK>" data/local/lang_test data/lang_test
...
#utils/subset_data_dir.sh --first data/train_all 10000 data/test_and_test
...
utils/subset_data_dir.sh data/train 200 data/train.1k  || exit 1;
...
steps/train_mono.sh --nj $njobs --cmd "$train_cmd" \
  data/train.1k data/lang exp/mono0a 
...
(
  ...
  steps/decode.sh --nj $njobs --cmd "$decode_cmd" --config conf/decode.config \
   exp/tri3a/graph data/test exp/tri3a/decode_test || exit 1;
)&
...
steps/align_fmllr.sh --nj $njobs --cmd "$train_cmd" \
  data/train data/lang exp/tri3a exp/tri3b_ali || exit 1;

steps/train_sat.sh  --cmd "$train_cmd" \
  5000 100000 data/train data/lang exp/tri3b_ali  exp/tri4a || exit 1;

(
  utils/mkgraph.sh data/lang_test exp/tri4a exp/tri4a/graph
  steps/decode_fmllr.sh --nj $njobs --cmd "$decode_cmd" --config conf/decode.config \
...
(
  ...
  --per-utt true --online true exp/nnet2_online/nnet_ms_a_online/graph/ \
  data/test/ exp/nnet2_online/nnet_ms_a_online/decode_test
)&
