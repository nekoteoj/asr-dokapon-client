    ...
    def opened(self):
        ...
        def send_data_to_ws():
            if self.send_adaptation_state_filename is not None:
                print(f"Sending adaptation state from \
            ...
            if 'adaptation_state' in response:
                if self.save_adaptation_state_filename:
                    print("Saving adaptation state \
                        to %s" % self.save_adaptation_state_filename)
...
def main():
    ...
    parser.add_argument(
        '--content-type',
        default='',
        help="Use the specified content type (empty by default, for raw files \
        the default is  audio/x-raw, layout=(string)interleaved, \
        rate=(int)<rate>, format=(string)S16LE, channels=(int)1"
    )
    parser.add_argument('audiofile',
                        help="Audio file to be sent to the server",
